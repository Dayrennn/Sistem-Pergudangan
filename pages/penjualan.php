...
if ((!empty($data_pelanggan) && !empty($data_pesanan)) || 
    ...
    if (!empty($data_pelanggan) && !empty($data_pesanan)) {
        ...
        // Data dari form penjualan.php (saat pesanan dibuat dari form)
...
<div class="container">
    ...
    <aside class="sidebar">
        <ul>
            ...
            <li><a href="penjualan.php"><i class='bx bx-cart'></i><span>Data Penjualan</span></a></li> <li><a href="supplier.php"><i class='bx bx-store-alt'></i><span>Data Supplier</span></a></li>
...
<script>
    // Di dalam file JS yang relevan (misalnya javascript/penjualan.js atau di <script> di penjualan.php)

// Contoh di javascript/penjualan.js (atau di <script> di penjualan.php)
    ...
    document.addEventListener('click', function(event) {
        ...
        if (event.target.classList.contains('btn-hapus-penjadwalan')) {
            ...
            if (confirm('Apakah Anda yakin ingin menghapus data penjadwalan pembelian ini?')) {
                fetch(`hapus_penjualan.php?id=${idToDelete}`, { // Panggil hapus_penjualan.php dengan ID via GET
                    method: 'GET', // Sesuai dengan hapus_penjualan.php yang pakai $_GET
                })
                .then(response => {
                    if (!response.ok) {
